...
if __name__ == "__main__":
    ...
    with open(result_txt_name, "w") as txt_file:
        ...
        for ground_image_info in ground_info_list:
            ...
            try:
                result_array = np.array(Image.open(corresponding_label_ids_file))[:, :, 0]
                ...
                image_average_acc = 0
                total_accs.append(image_average_acc)
                total_accuracy.append(accuracy)
                result_ids.append(file_name)
                input_line = f"mIoU:{image_average_iou} mAcc:{image_average_acc} accuracy:{accuracy}\n"
                txt_file.write(input_line)
                continue
            ...
            for one_category_id in filtered_ids_ground:
                ...
                TP = np.sum(np.logical_and(ground_pixels, result_pixels))
                FN = np.sum(np.logical_and(ground_pixels, ~result_pixels))
                FP = np.sum(np.logical_and(~ground_pixels, result_pixels))

                iou_for_one_category = TP / (TP + FP + FN)
                acc_for_one_category = TP / (TP + FN)
                image_ious.append(iou_for_one_category)
                image_accs.append(acc_for_one_category)

                input_line = f"    IoU for {one_category_id} - {full_103[one_category_id]} : {iou_for_one_category} \n"
                txt_file.write(input_line)
                input_line = f"    Acc for {one_category_id} - {full_103[one_category_id]} : {iou_for_one_category} \n"
                txt_file.write(input_line)

            image_average_iou = np.mean(image_ious)
            image_average_acc = np.mean(image_accs)
            total_icus.append(image_average_iou)
            total_accs.append(image_average_acc)
            total_accuracy.append(accuracy)
        ...
        df = pd.DataFrame(
            {'image ID': result_ids, 'mIoU': total_icus, 'mAcc': total_accs, 'Detect Accuracy': total_accuracy})
        df.to_excel(result_excel_name)
        mean_iou = np.mean(total_icus)
        mean_accuracy = np.mean(total_accuracy)
        mean_acc = np.mean(total_accs)
        ...
        print(f"mAcc: {mean_acc}")

        input_line = f"mIOU: {mean_iou} \n"
        txt_file.write(input_line)
        input_line = f"m Acc: P{mean_acc} \n"
